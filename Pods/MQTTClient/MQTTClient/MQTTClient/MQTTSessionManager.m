...
- (void) subscribeToTopic:(NSString *)topic {
    [self.session subscribeToTopic:topic atLevel:2];
}
...
- (void) sendMessage:(NSString *)topic message:(NSString *) message{
    NSData* cData = [message dataUsingEncoding:NSUTF8StringEncoding];
    [self.session publishData:cData onTopic:topic retain:false qos:1];
}

- (void) sendMessageSpecial:(NSString *)topic message:(NSString *) message{
    NSData* cData = [message dataUsingEncoding:NSUTF8StringEncoding];
    [self.session publishData:cData onTopic:topic retain:true qos:2];
}
